...
from commcarehq import Domain, privileges
...
def get_change_status(from_plan_version, to_plan_version):
    ...
    from commcarehq.apps.accounting.models import SubscriptionAdjustmentReason as Reason
...
def get_address_from_invoice(invoice):
    from commcarehq.apps.accounting.invoice_pdf import Address
    from commcarehq.apps.accounting.models import BillingContactInfo
    try:
...
def get_customer_cards(account, username, domain):
    ...
    from commcarehq.apps.accounting.payment_handlers import get_or_create_stripe_customer
    try:
