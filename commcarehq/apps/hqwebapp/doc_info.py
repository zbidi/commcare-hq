    ...
    if doc_type in ('Application', 'RemoteApp'):
        if doc.get('copy_of'):
            doc_info = DocInfo(
                ...
                link=reverse(
                    'commcarehq.apps.app_manager.views.download_index',
                    args=[domain, doc_id],
                ),
            )
        else:
            doc_info = DocInfo(
                ...
                link=reverse(
                    'commcarehq.apps.app_manager.views.view_app',
                    args=[domain, doc_id],
                ),
            )
        ...
        from commcarehq.apps.users.views.mobile import EditGroupMembersView
        doc_info = DocInfo(
