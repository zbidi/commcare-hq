...
from commcarehq.apps.adm.admin.crud import *
from commcarehq.apps.crud.models import AdminCRUDDocumentMixin
...
class BaseADMDocument(Document, AdminCRUDDocumentMixin):
    ...
    def get_correct_wrap(cls, docid):
        try:
            ...
            adm = to_function('commcarehq.apps.adm.models.%s' % adm_class)
...
class ConfigurableADMColumn(BaseADMColumn):
    ...
    def all_admin_configurable_columns(cls):
        ...
        for item in data:
            ...
            try:
                item_class = to_function("commcarehq.apps.adm.models.%s" % value.get('type', 'ADMColumn'))
