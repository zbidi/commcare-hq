...
from commcarehq.apps.crud.models import BaseAdminHQTabularCRUDManager
...
class DaysSinceColumnAdminCRUDManager(CouchViewColumnAdminCRUDManager):
    ...
    def format_property(self, key, property):
        if key == "start_or_end":
            from commcarehq.apps.adm.admin.forms import DATESPAN_CHOICES
...
class CaseCountColumnCRUDManager(ConfigurableColumnAdminCRUDManager):
    ...
    def format_property(self, key, property):
        ...
        if key == 'filter_option':
            from commcarehq.apps.adm.models import CASE_FILTER_OPTIONS
        ...
        if key == 'case_status':
            from commcarehq.apps.adm.models import CASE_STATUS_OPTIONS
...
class ADMReportCRUDManager(ADMAdminCRUDManager):
    ...
    def format_property(self, key, property):
        if key == 'column_refs':
            ol = ['<ol>']
            for ref in property:
                from commcarehq.apps.adm.models import BaseADMColumn
        ...
        if key == 'reporting_section':
            from commcarehq.apps.adm.models import REPORT_SECTION_OPTIONS
        ...
        if key == "sort_by_direction":
            from commcarehq.apps.adm.models import SORT_BY_DIRECTION_OPTIONS
