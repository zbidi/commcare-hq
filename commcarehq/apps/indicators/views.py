...
from commcarehq.apps.crud.views import BaseCRUDFormView
from commcarehq.apps.domain.decorators import login_and_domain_required
from commcarehq.apps.domain.views import DomainViewMixin
from commcarehq.apps.hqwebapp.views import BaseSectionPageView
from commcarehq.apps.indicators.admin.crud import IndicatorCRUDFormRequestManager
from commcarehq.apps.indicators.admin.forms import BulkCopyIndicatorsForm
from commcarehq.apps.indicators.dispatcher import require_edit_indicators
from commcarehq.apps.indicators.forms import ImportIndicatorsFromJsonFileForm
from commcarehq.apps.indicators.models import (
    IndicatorDefinition,
    DynamicIndicatorDefinition,
)
from commcarehq.apps.indicators.utils import get_indicator_domains, get_namespaces
...
def default_admin(request, domain, template="reports/base_template.html", **kwargs):
    if request.domain not in get_indicator_domains():
        raise Http404
    from commcarehq.apps.indicators.admin import BaseIndicatorAdminInterface
    context = dict(
        domain=domain,
        project=domain,
        report=dict(
...
class IndicatorAdminCRUDFormView(BaseCRUDFormView):
    base_loc = "commcarehq.apps.indicators.admin.forms"
...
class BulkCopyIndicatorsView(TemplateView):
    indicator_loc = "commcarehq.apps.indicators.models"
