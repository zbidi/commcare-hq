...
from commcarehq.apps.sms.util import (clean_phone_number, clean_text,
    get_available_backends)
from commcarehq.apps.sms.models import (SMSLog, OUTGOING, INCOMING,
    PhoneNumber, ERROR_PHONE_NUMBER_OPTED_OUT)
from commcarehq.apps.sms.messages import (get_message, MSG_OPTED_IN,
    MSG_OPTED_OUT, MSG_DUPLICATE_USERNAME, MSG_USERNAME_TOO_LONG)
from commcarehq.apps.sms.mixin import MobileBackend, VerifiedNumber, SMSBackend
from commcarehq.apps.domain.models import Domain
from datetime import datetime

from commcarehq.apps.sms.util import register_sms_contact, strip_plus
...
def enqueue_directly(msg):
    try:
        from commcarehq.apps.sms.management.commands.run_sms_queue import SMSEnqueuingOperation
...
def process_username(username, domain):
    ...
    """
    from commcarehq.apps.users.forms import validate_username
...
def create_billable_for_sms(msg, delay=True):
    if not msg.domain:
        return
    try:
        from commcarehq.apps.sms.tasks import store_billable
        if delay:
