...
from commcarehq.apps.app_manager.exceptions import UnknownInstanceError, ScheduleError
from commcarehq.apps.app_manager.templatetags.xforms_extras import trans
from commcarehq.apps.app_manager.const import CAREPLAN_GOAL, CAREPLAN_TASK, SCHEDULE_LAST_VISIT, SCHEDULE_PHASE, \
    CASE_ID, RETURN_TO
from commcarehq.apps.app_manager.util import split_path, create_temp_sort_column, languages_mapping
from commcarehq.apps.app_manager.xform import SESSION_CASE_ID, autoset_owner_id_for_open_case, \
    autoset_owner_id_for_subcase
from commcarehq.apps.app_manager.xpath import dot_interpolate, CaseIDXPath, session_var, \
    CaseTypeXpath, ItemListFixtureXpath, ScheduleFixtureInstance, XPath, ProductInstanceXpath
from commcarehq.apps.hqmedia.models import HQMediaMapItem
...
def get_default_sort_elements(detail):
    from commcarehq.apps.app_manager.models import SortElement
...
class SuiteGenerator(SuiteGeneratorBase):
    ...
    def build_detail(self, module, detail_type, detail, detail_column_infos,
        ...
        """
        from commcarehq.apps.app_manager.detail_screen import get_column_generator
    ...
    def build_case_tile_detail(self, module, detail, detail_type):
        ...
        string.
        """
        from commcarehq.apps.app_manager.detail_screen import get_column_xpath_generator
    ...
    def entries(self):
        ...
        from commcarehq.apps.app_manager.models import Module, AdvancedModule
        results = []
        for module in self.modules:
            for form in module.get_forms():
                e = Entry()
    ...
    def add_auto_select_datums_to_entry(self, action, auto_select, e, form):
        from commcarehq.apps.app_manager.models import AUTO_SELECT_USER, AUTO_SELECT_CASE, \
    ...
    def configure_entry_advanced_form(self, module, e, form, **kwargs):
        ...
        def get_target_module(case_type, module_id, with_product_details=False):
            if module_id:
                ...
                from commcarehq.apps.app_manager.models import ModuleNotFoundException
                try:
    ...
    def menus(self):
        # avoid circular dependency
        from commcarehq.apps.app_manager.models import CareplanModule, AdvancedForm
