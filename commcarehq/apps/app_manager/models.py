...
from commcarehq import toggles, privileges
from commcarehq.apps.app_manager.feature_support import CommCareFeatureSupportMixin
from commcarehq.util.quickcache import quickcache
from django_prbac.exceptions import PermissionDenied
from commcarehq.apps.accounting.utils import domain_has_privilege

from commcarehq.apps.app_manager.commcare_settings import check_condition
from commcarehq.apps.app_manager.const import *
from commcarehq.apps.app_manager.xpath import dot_interpolate, LocationXpath
from commcarehq.apps.builds import get_default_build_spec
from commcarehq.util.hash_compat import make_password
...
from commcarehq.util import bitly
from commcarehq.util import view_utils
from commcarehq.apps.appstore.models import SnapshotMixin
from commcarehq.apps.builds.models import BuildSpec, CommCareBuildConfig, BuildRecord
from commcarehq.apps.hqmedia.models import HQMediaMixin
from commcarehq.apps.reports.templatetags.timezone_tags import utc_to_timezone
from commcarehq.apps.translations.models import TranslationMixin
from commcarehq.apps.users.models import CouchUser
from commcarehq.apps.users.util import cc_user_domain
from commcarehq.apps.domain.models import cached_property
from commcarehq.apps.app_manager import current_builds, app_strings, remote_app
from commcarehq.apps.app_manager import suite_xml, commcare_settings
from commcarehq.apps.app_manager.util import split_path, save_xform, get_correct_app_class, ParentCasePropertyBuilder
from commcarehq.apps.app_manager.xform import XForm, parse_xml as _parse_xml, \
    validate_xform
...
class Form(IndexedFormBase, NavMenuItemMediaMixin):
    ...
    def update_app_case_meta(self, app_case_meta):
        from commcarehq.apps.reports.formdetails.readable import FormQuestionResponse
        questions = {
...
class ModuleBase(IndexedSchema, NavMenuItemMediaMixin):
    ...
    def validate_detail_columns(self, columns):
        from commcarehq.apps.app_manager.suite_xml import FIELD_TYPE_LOCATION
        from commcarehq.apps.locations.util import parent_child
        hierarchy = None
...
class ApplicationBase(VersionedDoc, SnapshotMixin,
    ...
    def ota_restore_url(self):
        return reverse('commcarehq.apps.ota.views.restore', args=[self.domain])
    ...
    @absolute_url_property
    def jar_url(self):
        return reverse('commcarehq.apps.app_manager.views.download_jar', args=[self.domain, self._id])
    ...
    def odk_profile_url(self):
        return reverse('commcarehq.apps.app_manager.views.download_odk_profile', args=[self.domain, self._id])

    @absolute_url_property
    def odk_media_profile_url(self):
        return reverse('commcarehq.apps.app_manager.views.download_odk_media_profile', args=[self.domain, self._id])
        ...
        try:
            ...
            if settings.BITLY_LOGIN:
                copy.short_url = bitly.shorten(
                    get_url_base() + reverse('commcarehq.apps.app_manager.views.download_jad', args=[copy.domain, copy._id])
                )
                copy.short_odk_url = bitly.shorten(
                    get_url_base() + reverse('commcarehq.apps.app_manager.views.download_odk_profile', args=[copy.domain, copy._id])
                )
                copy.short_odk_media_url = bitly.shorten(
                    get_url_base() + reverse('commcarehq.apps.app_manager.views.download_odk_media_profile', args=[copy.domain, copy._id])
                )
        except AssertionError:
            raise
...
def validate_detail_screen_field(field):
    ...
    # commcarehq/apps/app_manager/static/app_manager/js/detail-screen-config.js
...
class Application(ApplicationBase, TranslationMixin, HQMediaMixin):
    ...
    def save(self, *args, **kwargs):
        ...
        from commcarehq.apps.app_manager import signals
    ...
    def get_case_metadata(self):
        from commcarehq.apps.reports.formdetails.readable import AppCaseMetadata
