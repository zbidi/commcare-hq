...
def _get_location_or_not_exist(location_id, domain):
    try:
        return SQLLocation.objects.get(location_id=location_id, domain=domain)
    except SQLLocation.DoesNotExist:
        return object_does_not_exist('Location', location_id)
...
class LocationResource(HqBaseResource):
    ...
    def obj_get(self, bundle, **kwargs):
        ...
        return _get_location_or_not_exist(location_id, domain)
    ...
    def obj_get_list(self, bundle, **kwargs):
        ...
        if not parent_id:
            ...
            if not user_can_access_location_id(kwargs['domain'], user, parent_id):
                raise BadRequest(LOCATION_ACCESS_DENIED)
            parent = _get_location_or_not_exist(parent_id, domain)
